...
static const struct backlight_ops kb9202bl_ops = {
	.get_brightness	= kb9202bl_get_intensity,
	.update_status	= kb9202bl_set_intensity,
};

static int kb9202bl_probe(struct platform_device *pdev)
{
	struct backlight_properties props;
	struct backlight_device *bd;

	memset(&props, 0, sizeof(struct backlight_properties));
	props.max_brightness = KB9202_MAX_INTENSITY;
	props.brightness = KB9202_DEFAULT_INTENSITY;

	bd = backlight_device_register("kb9202-bl", &pdev->dev, NULL, &kb9202bl_ops, &props);
	if (IS_ERR(bd))
		return PTR_ERR(bd);
...
static struct platform_driver kb9202bl_driver = {
	.remove		= __exit_p(kb9202bl_remove),
...
static int __init kb9202bl_init(void)
{
	...
	ret = platform_driver_probe(&kb9202bl_driver, kb9202bl_probe);
	if (!ret) {
